...
void add_job(pid_t pid, const char* command) {
    for (int i = 0; i < MAX_BG_JOBS; i++) {
        if (!bg_jobs[i].is_active) {
            ...
            snprintf(bg_jobs[i].co#include<stdio.h>
#include<stdlib.h>
#include<unistd.h>
#include<string.h>
#include<signal.h>
...
void process_single_command(char* command_str, char* currpath, char* homepath) {
    ...
    // Check for BUILT-IN commands first. They NEVER run in the background.
    ...
    // If it's not a built-in, it's an EXTERNAL command.
        ...
        // Now we pass the is_background flag down to the execution functions.
...
int main(){
    ...
    // Very conservative job control - only in fully interactive terminals
    if (isatty(STDIN_FILENO) && isatty(STDOUT_FILENO) && getenv("TERM") != NULL && 
        strstr(getenv("TERM"), "xterm") != NULL) {
        
        // Try to set up job control, but don't fail if it doesn't work
        if (setpgid(shell_pgid, shell_pgid) == 0) {
            // Only proceed if we can actually control the terminal
            if (tcgetpgrp(STDIN_FILENO) != -1) {
                ...
                // Try to take control of terminal
                tcsetpgrp(STDIN_FILENO, shell_pgid);
            }
        }
    } else {
        // Non-interactive mode: use default signal handling
